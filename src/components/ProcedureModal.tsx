...
function GcsProcedure({
  ...
  const completedIds = responses.map((item) => item.id);
  const verbalChecks = completedIds.filter((id) => id.startsWith("verbal-")).length;
  return (
    <div className="procedure-body">
      ...
      <SubmitButton
        disabled={
          ...
          !motor ||
          !total ||
          !completedIds.includes("eye-observe") ||
          !completedIds.includes("eye-normal-call") ||
          verbalChecks < 3 ||
          !completedIds.includes("motor-left-hand") ||
          !completedIds.includes("motor-right-hand")
        }
        onClick={submit}
      />
    </div>
